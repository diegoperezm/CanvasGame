...
function BadGuy(args) {
  (this.ID = args.id), (this.X = args.spawnX);
  this.Y = args.spawnY;
  ...
  while (l < col.length) {
    ...
    for (let i = 0; i < col.length; i++) {
      if (i !== l) {
        ...
        if (collision(matchingFor, matchingWith)) {
...
function collision(objOne, objTwo) {
  if (
    objOne.X > objTwo.X - objOne.W &&
    objOne.X < objTwo.X + objTwo.W &&
...
spawn(
  characters["Players"],
  createCharacter({ level: 1, spawnX: 25, spawnY: 25 }, Player)
);
